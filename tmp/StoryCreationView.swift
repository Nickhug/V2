...
struct StoryEditorView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                // Drawing canvas - should be above the image but below UI controls
    ...
    private func handleToolSelection(_ tool: EditingTool) {
        ...
        // Ensure proper tool selection with animation
        withAnimation(.easeInOut(duration: 0.25)) {
            if selectedEditTool == selectedTool {
                // If same tool selected, leave it selected
                // No change needed - keep the tool selected
            } else {
                // Toggle to new tool
    ...
    private var drawingCanvasOverlay: some View {
        // Always show the canvas, but only make it active when in draw mode
        ZStack {
            // Display completed paths regardless of edit mode
            Canvas { context, size in
                drawCompletedPaths(in: context)
            }
            
            // Only allow adding new paths when in draw mode
